...
pub fn get_config_dir() -> PathBuf {
  ...
  let config_file = appdata.join("sigmacord").join("config.json");

  if fs::metadata(appdata.join("sigmacord")).is_err() {
    fs::create_dir_all(appdata.join("sigmacord")).expect("Error creating appdata dir");
  }
